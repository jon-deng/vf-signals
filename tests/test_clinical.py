...
@pytest.fixture()
def y_is_closing(y_linear, y_slope):
    """
...
def test_is_closing(y_linear, time, y_is_closing):
    assert np.all(clinical.is_closing(y_linear, time, closed_ub=0) == y_is_closing)

@pytest.fixture()
def y_is_opening(y_linear, y_slope):
    """
